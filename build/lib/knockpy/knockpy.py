...
def main():
	parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter, prog='knockpy', description=__description__, epilog= __epilog__)
	parser.add_argument('domain', help='target to scan, like domain.com')
	parser.add_argument('-w', help='specific path to wordlist file', nargs=1, dest='wordlist', required=False)
	parser.add_argument('-r', '--resolve', help='resolve single ip or domain name', action='store_true', required=False)
	parser.add_argument('-c', '--csv', help='save output in csv', action='store_true', required=False)
	parser.add_argument('-f', '--csvfields', help='add fields name to the first row of csv output file', action='store_true', required=False)
	parser.add_argument('-j', '--json', help='export full report in JSON', action='store_true', required=False)
	...
	if zonetransfer_json['enabled']:
		init('YES', True)
		print(
			(json.dumps(zonetransfer_json['list'], indent=4, separators=(',', ': '))))
	...
	stats = {'time_start': time_start, 'time_end': time_end, 'sub_count': sub_count, 'ip_count': ip_count, 'wordlist': {'filename': wordlist, 'item_count': wordlist_count}, 'knockpy': {'version': __version__, 'query': sys.argv, 'url': __url__}}
	...
	if not resolve_host:
		if save_scan_csv:
			...
			report_json = {'target_response': response_resolve, 'subdomain_response': subdomains_json_list, 'found': {'ipaddress': ipaddr_list, 'subdomain': subdomain_found, 'csv': subdomain_csv_list}, 'info': stats}
